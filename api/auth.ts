...
import { serializeBigInt, syncSequence } from './lib/db-utils'

export default async function handler(req: VercelRequest, res: VercelResponse) {
  const params = { ...req.query, ...req.body }
  const { operation } = params as { operation: string }

  try {
    switch (operation) {
      case 'me': {
        const { auth_id } = params as { auth_id: string }
      ...
      case 'invite': {
        ...
        const existingAuthUser = listData?.users?.find(u => u.email?.toLowerCase() === (email as string).toLowerCase())
      ...
      case 'updateMe': {
        const { data, auth_id } = params as { data: any, auth_id: string }
